...
def already():
    state.clear()
    state.color("green")
    state.write("You already tried that!!", font=("Arial",30,"bold"))

def wrong():
    global wrongCount
    state.clear()
    state.color("red")
    state.write(str(9-wrongCount), font=("Arial",30,"bold"))
    state.write("   Attemps left", font=("Arial",30,"bold"))
...
def start_game():
    ...
    global gameFlag
    global movie
    global movie_space_position
    global user_list_correct
    global user_list_wrong
    global user_input
    global letter
...
def evaluate(movie,user_input):
    ...
    if(gameFlag==True):
        if(user_input in movie and user_input not in user_list_correct):
            ...
            if(user_input in user_list_correct or user_input in user_list_wrong):
                already()
...
def continue_game():
    ...
    temp3.setpos(0,-100)
    temp3.pendown()
    temp3.write("To start game press 'Enter'. To exit press 'space'.", font=("Arial",20,"bold"))
    wn.listen()
